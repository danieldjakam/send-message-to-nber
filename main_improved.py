# -*- coding: utf-8 -*-
"""
Application Excel vers WhatsApp - Version améliorée avec architecture modulaire

Cette application permet de lire des fichiers Excel et d'envoyer les données
via WhatsApp en utilisant l'API UltraMsg avec des fonctionnalités avancées:
- Sécurité renforcée avec chiffrement des tokens
- Validation stricte des numéros de téléphone
- Envois asynchrones avec threading
- Logging structuré
- Interface utilisateur moderne et réactive

Auteur: Application améliorée par Claude
Version: 2.0
"""
...
class ExcelWhatsAppApp:
    """Application principale Excel vers WhatsApp avec architecture améliorée"""
    
    def __init__(self, root):
        self.root = root
        self.root.title("📊 Excel vers WhatsApp Pro")
    ...
    def create_widgets(self):
        ...
        version_label = ctk.CTkLabel(
            title_frame,
            text="Version 2.0 - Architecture Sécurisée",
    ...
    def _create_api_section(self) -> CollapsibleSection:
        ...
        self.instance_entry = ValidatedEntry(
            ...
            validator=lambda x: DataValidator.validate_api_credentials(x, "dummy")[0:1] + ("Instance ID requis",),
            height=35
        )
    ...
    def _sync_ui_with_variables(self):
        ...
        # Synchroniser les champs avec les variables
    ...
    def load_file(self):
        ...
        try:
            ...
            messagebox.showinfo(
                ...
                f"Vous pouvez maintenant sélectionner les colonnes à inclure."
            )
            
        except FileNotFoundError:
            error_msg = "Fichier non trouvé"
            self.status_indicator.set_status('error', error_msg)
            logger.error("file_load_error", file_path=file_path, error=error_msg)
            messagebox.showerror("Erreur", error_msg)
        except PermissionError:
            error_msg = "Accès refusé au fichier"
            self.status_indicator.set_status('error', error_msg)
            logger.error("file_load_error", file_path=file_path, error=error_msg)
            messagebox.showerror("Erreur", error_msg)
        except Exception as e:
    ...
    def create_column_checkboxes(self):
        ...
        # Créer un conteneur avec colonnes multiples
        container = ctk.CTkFrame(self.columns_scroll_frame, fg_color="transparent")
        container.pack(fill='x', padx=5, pady=5)
        
        # Calculer le nombre de colonnes d'affichage selon la largeur
        ...
        # Distribuer les checkboxes
        for i, column in enumerate(self.df.columns):
            var = tk.BooleanVar(value=True)
            self.column_vars[column] = var
            
            # Déterminer le frame parent
            frame_index = i % num_display_cols
            parent_frame = display_frames[frame_index]
            
            # Créer le texte avec info sur le type de données
    ...
    def update_phone_column_options(self):
        ...
        if self.df is not None and not self.df.empty:
            ...
            # Tentative de sélection automatique intelligente
            auto_selected = False
            ...
            # Si aucune colonne n'a été détectée automatiquement
    ...
    def display_data(self, columns: List[str]):
        ...
        # Afficher la section de données
    ...
    def test_api_connection(self):
        ...
        try:
            ...
            # Vérifier qu'un fichier est chargé et qu'une colonne est sélectionnée
            if self.df is None or self.df.empty:
            ...
            # Trouver le premier numéro valide
            test_phone = None
            ...
            # Créer le client WhatsApp et tester
            ...
            if success:
                ...
                messagebox.showinfo(
                    ...
                    f"Vous pouvez maintenant procéder à l'envoi en masse."
                )
            else:
    ...
    def start_bulk_send(self):
        """Démarre l'envoi en masse avec validation complète"""
        if self.is_sending:
    ...
    def _validate_bulk_send_data(self) -> List[str]:
        ...
        # Validation des identifiants API
        instance_id = self.instance_id.get().strip()
        token = self.token.get().strip()
        ...
        # Validation de la colonne téléphone
        ...
        # Validation des colonnes de données si nécessaire
    ...
    def _execute_bulk_send(self, messages_data: List[tuple]):
        """Exécute l'envoi en masse dans un thread séparé"""
        try:
            ...
            if not self.whatsapp_client:
                self.whatsapp_client = WhatsAppClient(
                    ...
                    rate_limit=0.8  # Limite à 0.8 message par seconde
                )
            ...
            # Exécuter l'envoi
            results = self.whatsapp_client.send_bulk_messages(
                messages_data,
                max_workers=2,  # Limite le nombre de threads
                progress_callback=progress_callback
            )
            
            # Traiter les résultats dans le thread principal
            self.root.after(0, lambda: self._handle_bulk_send_results(results))
        
        except Exception as e:
            error_msg = f"Erreur lors de l'envoi: {str(e)}"
            logger.error("bulk_send_execution_error", error=str(e))
            self.root.after(0, lambda: self._handle_bulk_send_error(error_msg))
    
    def _handle_bulk_send_results(self, results: List[MessageResult]):
        """Traite les résultats de l'envoi en masse"""
        try:
            # Calculer les statistiques
            stats = self.whatsapp_client.get_sending_statistics(results)
            ...
            # Afficher le rapport
            self._show_sending_report(stats, results)
            
            # Mettre à jour le statut
            if stats['failed'] == 0:
                self.status_indicator.set_status('success', f"Envoi terminé: {stats['successful']}/{stats['total']}")
            else:
                self.status_indicator.set_status('warning', f"Envoi terminé avec erreurs: {stats['successful']}/{stats['total']}")
        
        except Exception as e:
            logger.error("bulk_send_results_error", error=str(e))
            self._handle_bulk_send_error(f"Erreur lors du traitement des résultats: {str(e)}")
    
    def _handle_bulk_send_error(self, error_msg: str):
        """Gère les erreurs d'envoi en masse"""
        self.progress_frame.hide()
        self.is_sending = False
        self.send_btn.configure(text="📨 Envoyer", state="normal")
    ...
    def _show_sending_report(self, stats: Dict, results: List[MessageResult]):
        """Affiche le rapport d'envoi détaillé"""
        ...
        if stats['failed'] == 0:
            ...
            # Avec erreurs
            error_details = []
            error_counts = stats['error_breakdown']
            
            for error, count in list(error_counts.items())[:5]:  # Top 5 erreurs
                error_details.append(f"• {error}: {count} fois")
            
            if len(error_counts) > 5:
                remaining = sum(list(error_counts.values())[5:])
                error_details.append(f"• Autres erreurs: {remaining} fois")
    ...
    def load_config(self):
        ...
        try:
            # Charger via le gestionnaire de configuration
            config = self.config_manager.get_all()
            
            # Restaurer les valeurs dans les variables
            ...
            # Restaurer l'état de la section API
    ...
    def on_config_change(self, *args):
        ...
        # Utiliser after_idle pour éviter les sauvegardes trop fréquentes
    ...
    def on_closing(self):
        ...
        try:
            ...
            # Arrêter les envois en cours si nécessaire
            if self.is_sending:
                self.is_sending = False
...
def main():
    ...
    try:
        ...
        logger.info("app_mainloop_started")
        root.mainloop()
