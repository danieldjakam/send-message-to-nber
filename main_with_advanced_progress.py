...
class ExcelWhatsAppApp:
    ...
    def create_widgets(self):
        ...
        self.status_indicator = StatusIndicator(
            self.root, font=ctk.CTkFont(size=12, weight="bold")
        )
    ...
    def _create_api_section(self) -> CollapsibleSection:
        ...
        ctk.CTkLabel(
            left_frame, text="🔑 Instance ID:", font=ctk.CTkFont(size=12, weight="bold")
        ...
        ctk.CTkLabel(
            right_frame, text="🔐 Token API:", font=ctk.CTkFont(size=12, weight="bold")
        ).pack(anchor="w", pady=(0, 5))
        self.token_entry = ValidatedEntry(
            right_frame,
            placeholder_text="Votre Token API...",
            validator=lambda x: (len(x.strip()) >= 10, "Token trop court")
            if x.strip()
            else (True, ""),
            height=35,
        )
        ...
        # Status de connexion
        self.api_status = StatusIndicator(control_frame, font=ctk.CTkFont(size=11))
        ...
        # Bouton de gestion des doublons
        history_btn = ctk.CTkButton(
            action_buttons,
            text="📝 Historique",
            command=self.show_duplicate_manager,
            height=35,
            width=90,
            font=ctk.CTkFont(size=10, weight="bold"),
            fg_color=("#8b8b8b", "#404040"),
        )
    ...
    def _create_columns_section(self) -> ctk.CTkFrame:
        ...
        self.columns_scroll_frame = ctk.CTkScrollableFrame(
            columns_frame, height=120, corner_radius=10
        )
    ...
    def load_file_with_progress(self):
        ...
        def load_thread():
            try:
                ...
                is_valid, validation_message = DataValidator.validate_excel_file(
                    file_path
                )
                if not is_valid:
                    self.root.after(
                        0, lambda: self._handle_file_error(validation_message)
                    )
                    return
                ...
                if self.df.empty:
                    self.root.after(
                        0, lambda: self._handle_file_error("Le fichier Excel est vide")
                    )
                    return
                ...
                self.root.after(
                    0,
                    lambda: self._handle_file_error(
    ...
    def _finalize_file_load(self, file_path: str):
        ...
        try:
            ...
            self.status_indicator.set_status(
                "success", f"Fichier chargé: {len(self.df)} lignes"
            )
    ...
    def show_duplicate_manager(self):
        ...
        duplicate_window.title("📝 Gestion des Doublons")
        duplicate_window.geometry("500x400")
        duplicate_window.transient(self.root)
        duplicate_window.grab_set()
        
        # Centrer la fenêtre
        duplicate_window.after(100, lambda: duplicate_window.focus())
        
        # Frame principale
        main_frame = ctk.CTkFrame(duplicate_window, corner_radius=15)
        main_frame.pack(fill="both", expand=True, padx=20, pady=20)
        
        # Titre
        title_label = ctk.CTkLabel(
            main_frame, 
            text="📊 Statistiques des Envois", 
            font=ctk.CTkFont(size=18, weight="bold")
        )
        title_label.pack(pady=(20, 10))
        
        # Obtenir les statistiques
        stats = self.bulk_sender.get_duplicate_stats()
        ...
        stats_text = f"""
📞 Numéros contactés: {stats['sent_numbers_count']}
📨 Messages envoyés: {stats['total_messages_sent']}
        """
        ...
        export_btn = ctk.CTkButton(
            ...
            command=lambda: self.export_sent_numbers(duplicate_window),
            height=40,
            font=ctk.CTkFont(size=12, weight="bold"),
            fg_color=("#1f538d", "#14375e")
        )
        export_btn.pack(side="left", padx=(0, 10), fill="x", expand=True)
        
        # Bouton reset
        reset_btn = ctk.CTkButton(
            buttons_frame,
            text="🗑️ Effacer l'historique",
            command=lambda: self.reset_sent_history(duplicate_window),
            height=40,
            font=ctk.CTkFont(size=12, weight="bold"),
            fg_color=("#d32f2f", "#b71c1c")
        )
        reset_btn.pack(side="right", padx=(10, 0), fill="x", expand=True)
    ...
    def export_sent_numbers(self, parent_window):
        """Exporte la liste des numéros contactés"""
        from tkinter import filedialog, messagebox
        
        file_path = filedialog.asksaveasfilename(
            parent=parent_window,
            title="Exporter la liste des numéros",
        ...
        if file_path:
            try:
                self.bulk_sender.export_sent_numbers(file_path)
    ...
    def reset_sent_history(self, parent_window):
        """Remet à zéro l'historique des envois après confirmation"""
        from tkinter import messagebox
        
        confirm = messagebox.askyesno(
            "⚠️ Confirmation",
            "Êtes-vous sûr de vouloir effacer tout l'historique?\n\n"
            "Cette action est irréversible et permettra de renvoyer\n"
            "des messages aux numéros précédemment contactés.",
            parent=parent_window
        )
        
        if confirm:
            try:
                self.bulk_sender.reset_duplicate_history()
                messagebox.showinfo("✅ Historique effacé", 
                                  "L'historique des envois a été remis à zéro.",
                                  parent=parent_window)
                parent_window.destroy()  # Fermer la fenêtre
            except Exception as e:
    ...
    def show_selected_data(self):
        ...
        if not selected_columns:
            ...
            messagebox.showwarning(
                "Attention", "Veuillez sélectionner au moins une colonne."
            )
            return
    ...
    def test_api_connection(self):
        ...
        try:
            ...
            is_valid, validation_msg = DataValidator.validate_api_credentials(
                instance_id, token
            )
            if not is_valid:
            ...
            if self.df is None or self.df.empty:
                ...
                messagebox.showwarning(
                    "⚠️ Aucune donnée", "Veuillez d'abord charger un fichier Excel."
                )
                return
            ...
            if not phone_column_name or phone_column_name not in self.df.columns:
                ...
                messagebox.showwarning(
                    "⚠️ Colonne manquante",
                    "Veuillez sélectionner une colonne de numéros de téléphone.",
                )
                return
            ...
            if not test_phone:
                ...
                messagebox.showerror(
                    "❌ Aucun numéro valide",
                    "Aucun numéro de téléphone valide trouvé dans la colonne sélectionnée.",
                )
                return
            ...
            if success:
                ...
                messagebox.showerror(
                    "❌ Test échoué", f"Le test a échoué:\n\n{message}"
                )
    ...
    def start_bulk_send_optimized(self):
        ...
        if self.is_sending:
            messagebox.showwarning(
                "⚠️ Envoi en cours", "Un envoi est déjà en cours. Veuillez patienter."
            )
            return
        ...
        try:
            ...
            if not messages_data:
                messagebox.showerror(
                    "❌ Aucune donnée", "Aucun message à envoyer après validation."
                )
                return
            
            # Vérification de la limite anti-spam
            MAX_MESSAGES = 2500
            if len(messages_data) > MAX_MESSAGES:
                # Calculer le nombre de sessions nécessaires
                total_sessions = (len(messages_data) + MAX_MESSAGES - 1) // MAX_MESSAGES
                
                warning_msg = (
                    f"⚠️ ATTENTION - LIMITE ANTI-SPAM\n\n"
                    f"Vous tentez d'envoyer {len(messages_data)} messages\n"
                    f"Limite de sécurité: {MAX_MESSAGES} messages par session\n\n"
                    f"Options disponibles:\n\n"
                    f"🔸 OPTION 1 - Envoi partiel (Recommandé)\n"
                    f"   Envoyer seulement les premiers {MAX_MESSAGES} messages\n\n"
                    f"🔸 OPTION 2 - Division en sessions\n"
                    f"   Diviser en {total_sessions} sessions de {MAX_MESSAGES} max\n"
                    f"   (Nécessite plusieurs lancements)\n\n"
                    f"Choisir Option 1 (Oui) ou Option 2 (Non) ?"
                )
                
                use_partial = messagebox.askyesno("🚨 Choix Anti-Spam", warning_msg)
                
                if use_partial:
                    # Option 1: Envoi partiel
                    messagebox.showinfo(
                        "📊 Envoi partiel", 
                        f"L'envoi sera limité aux premiers {MAX_MESSAGES} messages\n"
                        f"Les {len(messages_data) - MAX_MESSAGES} restants seront ignorés."
                    )
                else:
                    # Option 2: Montrer comment diviser
                    info_msg = (
                        f"📋 DIVISION EN SESSIONS\n\n"
                        f"Pour envoyer tous vos {len(messages_data)} messages:\n\n"
                        f"1️⃣ Cette session enverra les {MAX_MESSAGES} premiers\n"
                        f"2️⃣ Relancez l'envoi {total_sessions - 1} fois pour le reste\n\n"
                        f"Le système anti-doublons évitera les répétitions.\n\n"
                        f"Continuer avec la première session ?"
                    )
                    
                    continue_first = messagebox.askyesno("📋 Première Session", info_msg)
                    if not continue_first:
                        return
            elif len(messages_data) > 1000:
                # Avertissement pour les gros volumes même sous la limite
                warning_msg = (
                    f"📊 GROS VOLUME DÉTECTÉ\n\n"
                    f"Vous allez envoyer {len(messages_data)} messages\n\n"
                    f"Recommandations:\n"
                    f"• L'envoi peut prendre du temps\n"
                    f"• Surveillez les logs pour les erreurs\n"
                    f"• Évitez de fermer l'application\n\n"
                    f"Continuer avec cet envoi ?"
                )
                
                confirm = messagebox.askyesno("📊 Gros Volume", warning_msg)
                if not confirm:
                    return
    ...
    def _start_detailed_bulk_send(self, messages_data: List[tuple]):
        ...
        try:
            ...
            self.detailed_progress = DetailedProgressDialog(
                self.root, f"Envoi en Masse - {len(messages_data)} messages"
            )
    ...
    def _start_simple_bulk_send(self, messages_data: List[tuple]):
        ...
        thread = threading.Thread(
            target=self._execute_simple_bulk_send, args=(messages_data,), daemon=True
        )
        thread.start()
    ...
    def _execute_detailed_bulk_send(self, messages_data: List[tuple]):
        ...
        try:
            ...
            def progress_callback(completed, total, status):
                self.root.after(
                    0,
                    lambda: self.detailed_progress.update_progress(
    ...
    def _execute_simple_bulk_send(self, messages_data: List[tuple]):
        ...
        try:
            ...
            self.root.after(
                0, lambda: self.progress_frame.show("Envoi des messages...")
            )

            def progress_callback(completed, total, status):
                self.root.after(
                    0, lambda: self.progress_frame.update(completed, total, status)
                )
            ...
            session = self.bulk_sender.send_bulk_optimized(
                ...
                status_callback=lambda msg: self.root.after(
                    0, lambda: self.progress_frame.progress_label.configure(text=msg)
                ),
            )
    ...
    def _handle_detailed_bulk_send_results(self, session):
        ...
        try:
            ...
            if session.failed == 0:
                ...
                self.status_indicator.set_status(
                    "success",
                    f"Envoi terminé: {session.successful}/{session.total_messages}",
                )
            else:
                self.detailed_progress.add_log(
                    f"⚠️ Envoi terminé avec {session.failed} erreurs"
                )
                self.status_indicator.set_status(
                    "warning",
                    f"Envoi terminé avec erreurs: {session.successful}/{session.total_messages}",
                )

        except Exception as e:
            logger.error("detailed_bulk_send_results_error", error=str(e))
            self._handle_detailed_bulk_send_error(
                f"Erreur lors du traitement des résultats: {str(e)}"
            )
    ...
    def _handle_simple_bulk_send_results(self, session):
        ...
        try:
            ...
            self._handle_simple_bulk_send_error(
                f"Erreur lors du traitement des résultats: {str(e)}"
            )
        ...
        for idx, row in self.df.iterrows():
            ...
            try:
                if isinstance(phone_value, (int, float)):
                    ...
                    phone_raw = (
                        f"{phone_value:.0f}" if phone_value == phone_value else ""
                    )
                else:
            ...
            if selected_columns:
                ...
                for col in selected_columns:
                    try:
                        ...
                        logger.warning(
                            "data_line_creation_error", col=col, error=str(e)
                        )
                        continue
    ...
    def save_config(self):
        ...
        try:
            config_data = {
                ...
                "api_visible": getattr(
                    self.api_section, "is_open", tk.BooleanVar()
                ).get()
